...
def pose_estimate(cfg):
    ...
    # 2-生成环形传感器阵列的位置和方向
    beta1 = np.array([40.0,  82.5,  101.9, 129.5,  140.7,  185.3], dtype=np.double)   # 必须要浮点型, 否则
    beta2 = np.array([-36.2, -86.5, -96.6, -135.6, -152.6, -164.1], dtype=np.double)
    sensor_pos, M = get_array_pose(beta1, beta2)

    # 计算位置前的准备
    gt_mag_prop_arr = np.zeros((0, 7))
    est_mag_prop_arr = np.zeros((0, 7))
    for data_sensor, data_prop in train_loader:
        if data_sensor.shape[0] != 1:
            print("please set batch_size to 1")
            exit()
        data_sensor, data_prop = data_sensor[0, :].numpy(), data_prop[0, :].numpy()
        B_m = data_sensor.reshape(-1, 3)*1e-6 # uT -> T (real)

        # 磁铁位姿的真实值和初始值
        gt_mag_prop = data_prop
        init_mag_prop = np.append(gt_mag_prop, np.array([0, 0, 0])) # 加上地磁初始估计值
        ...
        print("mag_prop: ", est_mag_prop[0:7])
        print("Geo: ", est_mag_prop[7:10])
        est_mag_prop = est_mag_prop[0:7]       # 取部分结果

        # 添加到列表中
        gt_mag_prop_arr = np.append(gt_mag_prop_arr, gt_mag_prop.reshape(-1, 7), axis=0)
        est_mag_prop_arr = np.append(est_mag_prop_arr, est_mag_prop.reshape(-1, 7), axis=0)
...
def args_cfg():
    ...
    parser.add_argument('--key_word', type=str, default="real_15_15")
