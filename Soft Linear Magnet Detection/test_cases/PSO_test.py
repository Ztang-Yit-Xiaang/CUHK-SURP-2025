...
def fun_dipole_model(init_mag_prop):
    '''
    注意点: 
    ...
    B_m = np.zeros_like(raw_sensor_data)
    for i in range(sensor_ori.shape[0]):
        B_m[i] = np.dot(sensor_ori[i], raw_sensor_data[i].T).T
...
def args_cfg():
    ...
    parser.add_argument('--key_word', type=str, default="data_once_120815_l1.txt")
...
if __name__ == "__main__":
    ...
    for timestamp, data_sensor in train_loader:
        raw_sensor_data = data_sensor.numpy().reshape(-1, 3)*1e-6   # uT -> T (real)

        # 阈值判定
        if(np.sum(np.abs(raw_sensor_data))*1e6 < 500):
            print("continue")
            continue

        # lb = [-0.20, -0.10, 0.02, -1, -1, -1,  1.0e-08, -30e-6, -30e-6, -30e-6,\
        #        40, 70,  100,  125, 145, -80, -110, -140, -165, -185]
        # ub = [ 0.20,  0.10, 0.25,  1,  1,  1,  5.0e-08,  30e-6,  30e-6,  30e-6,\
        #        80, 110, 140,  165, 185, -40, -70, -100, -125, -145]
        lb = [-0.20, -0.10, 0.02, -1, -1, -1,  1.0e-08, -30e-6, -30e-6, -30e-6,\
               80, 80,  80,  170, 170, -100, -100, -100, -100, -100]
        ub = [ 0.20,  0.10, 0.25,  1,  1,  1,  3.0e-08,  30e-6,  30e-6,  30e-6,\
               100, 100, 100,  190, 190, -80, -80, -80, -80, -80]
        pso = PSO(func=fun_dipole_model, n_dim=20, pop=400, max_iter=150, lb=lb, ub=ub, w=0.8, c1=0.5, c2=0.5)
        pso.run()
