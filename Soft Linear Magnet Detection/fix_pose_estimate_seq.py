...
def save_process(cfg, est_mag_pose, clean=False):
    # 如果文件夹不存在，则创建
    if not os.path.exists(cfg.save_dir):
        os.makedirs(cfg.save_dir)

    save_file_name = cfg.key_word.replace("data", "fix_pose")
    save_file_path = os.path.join(cfg.save_dir, save_file_name)
    if os.path.exists(save_file_path) and clean:
        os.remove(save_file_path)
        return
    
    if est_mag_pose is None:
        return

    with open(save_file_path, 'a+') as f:
        np.savetxt(f, est_mag_pose, fmt="%s", delimiter=",")
    # np.savetxt(save_file_path, est_mag_pose, delimiter=",")

def pose_estimate(cfg):
    
    # 1-载入传感器采集的数据
    train_dataset = MagneticDatasetSeq(dir_path=cfg.dataset_dir, file_name=cfg.key_word, data_len=225, idx_offset=1000)
    train_loader = DataLoader(dataset=train_dataset, batch_size=1, shuffle=False, num_workers=0)

    # 2-生成环形传感器阵列的位置和方向
    beta1 = np.array([61.54,  89.16,  119.51, 145.47,  164.25], dtype=np.double)   # 必须要浮点型, 否则
    beta2 = np.array([-67.83, -81.36, -116.65, -146.32, -168.78], dtype=np.double)
    sensor_pos, M = get_array_pose(beta1, beta2)

    # 3-第一个点的磁铁位姿初值 商用胶囊Bt=2.37e-8  N52 Bt=7.94e-08
    init_mag_prop = np.array([0.009, 0.017, 0.17, -1, 0, 0, 2.37e-08, 0, 0, 0])
    # init_mag_prop = np.array([0.05296873242978198,-0.024317930838621132,0.17616422439545054,1,0,0,2.202555483044194e-08,1.6108959094821017e-06,2.649726637531546e-08,-1.3724760682581869e-06])

    est_mag_prop_arr = np.zeros((0, init_mag_prop.shape[0]+1))
    for idx, (timestamp, data_sensor) in enumerate(tqdm(train_loader, mininterval=10.0)):
        B_m = data_sensor.numpy().reshape(-1, 3)*1e-6 # uT -> T (real)

        # 阈值判定
        if(np.sum(np.abs(B_m))*1e6 < 500):
            print("idx=%d: continue" % (idx))
            continue
        ...
        # 通过最小二乘法计算位姿
        tic = timer()
        res_lsq = least_squares(fun_dipole_model, init_mag_prop, method="lm", args=(sensor_pos, B_m))
        print("idx=%d: cost time: %f (s)" % (idx, timer() - tic))
        ...
        # 添加到列表中
        str_mag_prop = np.append(timestamp, est_mag_prop.astype(str))
        est_mag_prop_arr = np.append(est_mag_prop_arr, str_mag_prop.reshape(1, -1), axis=0)
    
        # 间隔性保存
        if idx % cfg.save_interval == 0:
            print("save idx = ", idx)
            save_process(cfg, est_mag_prop_arr)  # 追加保存
            est_mag_prop_arr = np.zeros((0, init_mag_prop.shape[0]+1))    # 清空
...
def args_cfg():
    ...
    parser.add_argument('--dataset_dir', type=str, default='./dataset/volunteer2')
    parser.add_argument('--key_word', type=str, default="data_sequ_110930.txt")
    parser.add_argument('--save_dir', type=str, default="./results/volunteer2")
    parser.add_argument('--save_interval', type=int, default=600)   # 600 -> 1minute
    return parser.parse_args()

if __name__ == "__main__":
    cfg = args_cfg()

    save_process(cfg, None, clean=True)     # 先删除已有文件
    est_mag_pose = pose_estimate(cfg)
    save_process(cfg, est_mag_pose, clean=False)
